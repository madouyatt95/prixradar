...
async function proxyConfiguration(proxyUrls: readonly string[]) {
  const cleaned = proxyUrls.map((url) => url.trim()).filter(Boolean);
  if (cleaned.length > 0) return new ProxyConfiguration({ proxyUrls: cleaned });
  if (Actor.getEnv().isAtHome === "1") {
...
export async function collectFacebookSocialSources(options: {
  ...
  if (options.sources.length === 0) throw new Error("Aucun groupe Facebook public actif.");
  const now = options.now ?? new Date();
  const cursorMilliseconds = Date.parse(options.cursorAt);
  if (!Number.isFinite(cursorMilliseconds)) throw new Error("Curseur Facebook invalide.");
  const resultLimit = Math.max(1, Math.min(40, options.resultLimitPerSource));
  const results = new Map<string, SocialSourceResult>();
  const proxies = await proxyConfiguration(options.proxyUrls);
  const sourceForRequest = (sourceId: unknown) => options.sources.find((candidate) => candidate.id === sourceId);
  const crawler = new PlaywrightCrawler({
    launchContext: { launcher: chromium, launchOptions: { headless: true } },
    maxConcurrency: 1,
    maxRequestsPerCrawl: options.sources.length,
    maxRequestRetries: 1,
    ...
    onSkippedRequest: async ({ url, reason }) => {
      const source = options.sources.find((candidate) => url.startsWith(candidate.url));
    ...
    requestHandler: async ({ page, request }) => {
      const source = sourceForRequest(request.userData.sourceId);
      if (!source) return;
      const loaded = new URL(page.url());
      const loadedHost = loaded.hostname.toLowerCase().replace(/^www\./u, "");
      const loadedGroup = /^\/groups\/(\d{6,20})/u.exec(loaded.pathname)?.[1];
      if ((loadedHost !== "facebook.com" && loadedHost !== "m.facebook.com") || loadedGroup !== source.groupId) {
        const loginRedirect = loadedHost === "facebook.com" || loadedHost === "m.facebook.com"
          ? /\/(?:login|checkpoint)(?:\/|$)/u.test(loaded.pathname)
          : false;
        results.set(source.id, { source, publications: [], loadedUrl: page.url(), errorCode: loginRedirect ? "FACEBOOK_LOGIN_REQUIRED" : "UNEXPECTED_REDIRECT" });
        return;
      }
      const raw = await visibleArticles(page, source.groupId);
      const unique = new Map<string, SocialPublication>();
      let parsedPublications = 0;
      for (const article of raw) {
        const publication = parseFacebookArticle(article, source, now);
        if (publication) parsedPublications += 1;
        if (publication && Date.parse(publication.publishedAt) >= cursorMilliseconds - 60_000) {
          unique.set(publication.externalId, publication);
        }
      }
      const bodyText = parsedPublications > 0
        ? ""
        : await page.locator("body").innerText({ timeout: 3_000 }).catch(() => "");
      const accessBlocked = /(?:temporarily blocked|temporairement bloqué|access denied|captcha)/iu.test(bodyText);
      const loginRequired = /(?:connectez-vous|connexion à facebook|log in to facebook|you must log in)/iu.test(bodyText);
      results.set(source.id, {
        source,
        publications: [...unique.values()].sort((left, right) => Date.parse(right.publishedAt) - Date.parse(left.publishedAt)).slice(0, resultLimit),
        loadedUrl: page.url(),
        errorCode: parsedPublications > 0 ? null : accessBlocked ? "ANTI_BOT_BLOCKED" : loginRequired ? "FACEBOOK_LOGIN_REQUIRED" : "NO_PUBLICATION_VISIBLE",
      });
    },
  ...
  await crawler.run(options.sources.map((source) => ({
    url: source.url,
    uniqueKey: `${source.id}:${now.toISOString().slice(0, 16)}`,
    userData: { sourceId: source.id },
  })));
  return options.sources.map((source) => results.get(source.id) ?? ({
    source,
    publications: [],
    loadedUrl: null,
