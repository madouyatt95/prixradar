...
export function AdminView() {
  ...
  async function toggleAmazonExclusion(segment: DiscoverySegment, family: keyof typeof AMAZON_EXCLUSION_LABELS) {
    ...
    const response = await fetch("/api/admin/discovery", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ id: segment.id, excludedFamilies: next }),
    });
    setMessage(response.ok ? "Catégories Amazon mises à jour." : "Modification impossible.");
    if (response.ok) await refresh();
  }
  ...
  if (state === "unconfigured") return <section className="admin-empty"><h1>Espace administrateur fermé</h1><p>Le radar public fonctionne normalement. Les commandes de pilotage restent fermées tant qu’une connexion administrateur sécurisée n’est pas présentée.</p><button className="secondary-button" onClick={() => void refresh()}>Vérifier mon accès</button></section>;
  ...
  return (
    <section className="view-section">
      <div className="page-heading"><div><span className="eyebrow">Administration sécurisée</span><h1>Centre de pilotage</h1><p>Couverture, budgets, coupe-circuits et qualité du référentiel produit.</p></div><button className="secondary-button" onClick={() => void refresh()}>Actualiser</button></div>
      ...
      <section className="admin-panel">
        <div className="section-label-row"><div><span className="eyebrow">Registre versionné</span><h2>Couverture mesurée par connecteur</h2></div><span>{coverage.filter((item) => item.effectiveStatus === "active").length} actifs · {coverage.filter((item) => item.effectiveStatus === "locked").length} accès requis · {coverage.filter((item) => item.effectiveStatus === "authorized_unverified").length} en validation · {coverage.filter((item) => item.effectiveStatus === "planned").length} prévus</span></div>
        ...
        <div className="admin-source-list">{coverage.map((item) => {
          const effectiveLabel = item.effectiveStatus === "active" ? "Actif" : item.effectiveStatus === "locked" ? "Accès requis" : item.effectiveStatus === "authorized_unverified" ? "Autorisé · validation" : item.effectiveStatus === "degraded" ? "Dégradé" : item.effectiveStatus === "planned" ? "Prévu" : "Disponible";
          ...
          const statusDetail = item.effectiveStatus === "locked" ? "Autorisation explicite requise par la politique de l’enseigne" : item.effectiveStatus === "authorized_unverified" ? "Accès enregistré · premier contrôle réussi requis" : item.effectiveStatus === "active" ? `Dernier contrôle réussi${item.lastSuccessAt ? ` · ${new Date(item.lastSuccessAt).toLocaleString("fr-FR")}` : ""}` : item.effectiveStatus === "degraded" ? `Accès enregistré · santé ${item.liveStatus ?? "à contrôler"}` : item.effectiveStatus === "planned" ? "Planifié" : item.accessMode === "public_web" ? "Collecte publique prête · premier contrôle sain requis" : item.estimatedCoveragePercent === null ? "Couverture à calibrer" : `${item.estimatedCoveragePercent} % des segments calibrés`;
          const accessLabel = item.accessMode === "public_web" ? "web public" : item.accessMode === "authorization_required" ? "accès restreint" : item.accessMode === "api" ? "API" : item.accessMode === "planned" ? "prévu" : "web direct";
          return <div className="admin-source-row" key={`${item.source}:${item.market}`}><div><strong>{item.displayName} · {item.market}</strong><small>{accessLabel} · adaptateur {item.adapterVersion} · {item.configuredSegments} segments ({item.calibratedSegments} calibrés) · {item.categories} catégories</small><em className={item.effectiveStatus === "active" && item.contractStatus === "passing" && item.uncalibratedSegments === 0 ? "" : "is-warning"}>{statusDetail} · contrat {item.contractStatus} · {item.frontier.queued} URL en file</em></div><span className={`source-status status-${effectiveTone}`}><i />{effectiveLabel}</span></div>;
        })}</div>
      </section>
