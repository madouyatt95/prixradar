...
export default function TransparencyClient() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}><p className={styles.eyebrow}>Des chiffres, pas une promesse vague</p><h1>Voici comment nous mesurons la qualité des alertes.</h1><p>Les cartes restent volontairement vides tant que l’échantillon est trop petit. Seules les alertes vérifiées entrent dans les calculs.</p></section>

    <section className={styles.panel}><div className={styles.sectionTitle}><div><p className={styles.eyebrow}>30 derniers jours</p><h2>Fiabilité observée</h2></div><span>{typeof sample.alerts === "number" ? `${sample.alerts} alertes vérifiées` : "Données insuffisantes"}</span></div><div className={styles.metrics}>{METRIC_LABELS.map(([key, label]) => { const value = measurement(metrics[key]); return <article key={key}><span>{label}</span><strong>{displayMetric(value)}</strong><small>{value.value === null ? `minimum ${value.minimum} mesures` : `${value.sample} mesures`}</small></article>; })}</div></section>
    ...
    <section className={styles.method}><h2>Règles de publication</h2><ol><li>Uniquement des alertes issues d’une source active.</li><li>Aucun taux public avant le nombre minimal de mesures.</li><li>Une alerte n’est positive ou négative qu’après retours exploitables.</li><li>La disponibilité est revérifiée à 5, 15 et 30 minutes.</li></ol><Link href="/">Revenir au radar</Link></section>
  </main>;
}
